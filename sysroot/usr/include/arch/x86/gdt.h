...
typedef struct GlobalDescriptorTableEntry
{
    uint16_t limit_low;
    uint16_t base_low;
    uint8_t  base_middle;
    ...
    uint8_t  granularity;
    uint8_t  base_high;
}__attribute__((packed)) GlobalDescriptorTableEntry_t; //Prevent GCC to create padding spaces or rearrange the structure

typedef struct GoblalDescriptorPointer
{
    uint16_t base;            //The total size of the GDT
    uint32_t address;         //The address of the first GDT entry
}__attribute__((packed)) GoblalDescriptorPointer_t;

	//This two variables will store the gdt and the gdt pointer
GlobalDescriptorTableEntry_t gdt[5];
GoblalDescriptorPointer_t gdt_pointer;
