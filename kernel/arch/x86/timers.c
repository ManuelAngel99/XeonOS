...
volatile size_t pic_timer_ticks = 0;
...
void timer_wait(size_t ticks)
{
    ...
    eticks = pic_timer_ticks + ticks;

    while(pic_timer_ticks < eticks)
    {
...
DateTime_t get_current_date(void)
{
	...
	*/
	...
	if(!(register_b & 0x04))
	{
		...
		current_date_time.hour = BCD_DECODE(current_date_time.hour) + HOUR_ADJUST;
